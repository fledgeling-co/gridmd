...
const scalarKey = (s) => {
  if (!s) return null;
  if (s.kind === 'number') return `n:${Math.round(s.value * 1e9) / 1e9}`;
...
const contentKey = (c) => {
  if (!c) return null;
  if (c.rich) return `rich:${c.rich.map((r) => r.text).join('')}`;
...
test('round-trip: gmd → xlsx → gmd′ lints clean and the core model matches', () => {
  ...
  for (const s1 of model1.sheets) {
    ...
    for (const t1 of s1.tables) {
      const t2 = s2.tables.find((t) => t.name === t1.name);
  ...
  const names1 = (model1.fm.names ?? []).map((n) => n.name).sort();
  const names2 = (model2.fm.names ?? []).map((n) => n.name).sort();
