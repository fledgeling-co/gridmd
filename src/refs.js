    ...
    if (parts.length === 2) {
      ...
      if (m) {
        ...
        if (c1 <= MAX_COL && c2 <= MAX_COL) {
          return { kind: 'cols', sheet, c1: Math.min(c1, c2), c2: Math.max(c1, c2) };
        }
      }
      ...
      if (m) {
        ...
        if (r1 <= MAX_ROW && r2 <= MAX_ROW) {
          return { kind: 'rows', sheet, r1: Math.min(r1, r2), r2: Math.max(r1, r2) };
        }
      }
    }
  }
...
export const refKey = (col, row) => `${col},${row}`;
