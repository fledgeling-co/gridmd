...
import { parseTarget, parseCell, refKey, numToCol } from '../refs.js';
import { parseScalar } from '../scalar.js';
import { isValidPartPath } from '../validate.js';
...
export function buildWorkbookModel(doc, { baseDir = '.' } = {}) {
  ...
  const expandPatch = (props) => {
    if (!props) return {};
    const { style, ...rest } = props;
    return style ? { ...resolveStyle(style), ...rest } : rest;
  };

  const carryBlock = (b, feature, note) => {
  ...
  for (const sheet of doc.sheets) {
    const meta = sheet.blocks.find((b) => b.type === 'fence' && b.kind === 'sheet')?.meta ?? {};
    const s = {
    ...
    const setContent = (col, row, content) => {
    ...
    function bodyContent(body, flow) {
      if (body.formula !== undefined) {
        const f = String(body.formula).replace(/^=/, '');
        const content = { formula: f, cse: false, cached: body.value !== undefined ? yamlScalar(body.value) : null };
    ...
    function applyTable(b) {
      ...
      b.rows.forEach((row, ri) => row.cells.forEach((text, ci) => {
        const sc = parseScalar(text);
        if (header && ri === 0 && sc.kind === 'text') columns.push(sc.value);
        if (sc.kind !== 'blank') setContent(a.col + ci, a.row + ri, scalarContent(sc));
      }));
      const totalRow = tm.total ? a.row + b.rows.length : null;
      if (tm.total) {
      ...
      for (const [colName, cprops] of Object.entries(tm.cols ?? {})) {
        ...
        const lastRow = totalRow ?? (a.row + b.rows.length - 1);
        for (let r = a.row + (header ? 1 : 0); r <= lastRow; r++) cellAt(a.col + ci, r).patches.push(patch);
      }
      const sortLevels = (tm.sort ?? []).filter((lvl) => (lvl?.by ?? 'value') === 'value');
      ...
      s.tables.push({
        ...
        headerRow: header, bodyRows: b.rows.length - (header ? 1 : 0),
...
function rangeEdgeProps(props, t, col, row) {
  const out = { ...props };
...
export function translateFormula(formula, dr, dc) {
  const colToNum = (s) => [...s].reduce((n, c) => n * 26 + c.charCodeAt(0) - 64, 0);
  let out = '';
  let i = 0;
  while (i < formula.length) {
    ...
    if (m && !/[A-Za-z0-9_.]/.test(prev)) {
      ...
      const col = cd === '$' ? colL : numToCol(Math.max(1, colToNum(colL) + dc));
