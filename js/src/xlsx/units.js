...
export function isoToSerial(iso, dateSystem = 1900) {
  let datePart = null;
  let timePart = null;
  if (/^\d{2}:/.test(iso)) timePart = iso;
  else [datePart, timePart] = iso.split('T');
...
export const pxToPt = (px) => Math.round((px * 72 / 96) * 100) / 100;

// cm → inches (page margins).
export const cmToInch = (cm) => Math.round((cm / 2.54) * 10000) / 10000;
...
export function resolveColor(value, themeColors = {}) {
  ...
  if (!m) return null;
  const base = themeColors[m[1]] ?? DEFAULT_THEME[m[1]];
...
export const BUILTIN_NUMFMT_IDS = new Map(Object.entries({
