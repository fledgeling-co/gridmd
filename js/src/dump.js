...
const scalarDump = (s) => {
  if (!s) return null;
  switch (s.kind) {
    case 'number': return { t: 'n', v: s.value };
    case 'boolean': return { t: 'b', v: s.value };
    case 'error': return { t: 'e', v: s.value };
    case 'date':
    case 'time': return { t: 'd', v: s.value };
...
export function dumpModel(model) {
  const out = {
    ...
    names: (model.fm.names ?? [])
      .map((n) => ({ name: n.name, ref: n.ref ?? null, formula: n.formula ?? null, value: n.value !== undefined ? String(n.value) : null }))
      ...
      cells: Object.fromEntries(
        [...s.cells.values()]
          ...
          .map((c) => {
            ...
            const ct = c.content;
            if (ct.rich) return [ref, { t: 'rich', v: ct.rich.map((r) => r.text).join('') }];
