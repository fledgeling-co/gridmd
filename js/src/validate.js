...
import { parseTarget, parseCell, refKey, MAX_COL, MAX_ROW } from './refs.js';
import { parseScalar } from './scalar.js';
import { RESERVED_KINDS } from './parser.js';
...
const isSafeLink = (v) => typeof v === 'string' && /^(https:\/\/|mailto:|#)/.test(v);

function isSafeImageSrc(v) {
...
export function isValidPartPath(p) {
...
export function validateDocument(doc) {
  ...
  // ctx: { target, addDef } inside a sheet; {} at workbook level.
  function validateFence(b, ctx) {
    const meta = b.meta ?? {};
    const pos = b.args.positional;
    const need = (cond, msg) => { if (!cond) err(b.line, `{${b.kind}} ${msg}`); };
  ...
  function validateSheet(sheet) {
    const defs = new Map(); // refKey -> line
    const spills = [];      // ranges from spill:/array: props
    const spillCaches = [];
    const sheetMetas = [];
    let chartsAtSheet = 0;
    let gridContent = 0;

    const addDef = (col, row, line, what) => {
    ...
    const target = (text, line, kinds, what) => {
