...
import { numToCol, parseTarget } from '../refs.js';
...
const AGG = new Map(Object.entries({
...
const SHOW_AS = new Map(Object.entries({
...
function resolveSource(source, tableIndex, getHeaderCells) {
...
export function buildPivotParts(pivot, { pivotId, cacheId, tableIndex, getHeaderCells, numFmtId }) {
  const meta = pivot.meta ?? {};
  const src = resolveSource(meta.source, tableIndex, getHeaderCells);
  if (!src) return null;
  const { fields } = src;
  const fieldIdx = (name) => fields.findIndex((f) => f.toLowerCase() === String(name).toLowerCase());

  const rows = (meta.rows ?? []).map((r) => fieldIdx(r.field)).filter((i) => i !== -1);
  const cols = (meta.cols ?? []).map((c) => fieldIdx(c.field)).filter((i) => i !== -1);
  const pages = (meta.filters ?? []).map((f) => fieldIdx(f.field)).filter((i) => i !== -1);
  const values = (meta.values ?? []).map((v) => ({ ...v, _idx: fieldIdx(v.field) })).filter((v) => v._idx !== -1);
  ...
  const pivotFields = fields.map((_, i) => {
    ...
    const isData = values.some((v) => v._idx === i);
